...
## Результаты
Первые результаты на validation-выборке:
| Модель | Accuracy | Precision | Recall | F1 | ROC-AUC | Примечание |
|--------|----------|-----------|--------|----|---------|------------|
| Baseline: Logistic Regression | 0.7969 | 0.6841 | 0.4838 | 0.5668 | 0.8521 | Простая baseline-модель без сложного feature engineering |
...
`LogisticRegression` используется как baseline-модель, а остальные модели сравниваются с ней в `03_experiments.ipynb`. Полная таблица метрик сохранена в `data/processed/metrics_cp1.csv`. Лучшая модель сохранена в `models/best_model_cp1.joblib`.

...
├── Dockerfile                  # Docker-окружение для воспроизводимого запуска
├── docker-compose.yml          # Запуск проверки проекта одной командой
...
Воспроизводимый запуск в Docker:
```bash
docker compose up --build
```

Эта команда собирает одинаковое Python-окружение по `requirements.txt` и запускает тесты проекта внутри контейнера.
...
Результаты на validation-выборке после подбора гиперпараметров:
| Модель | Accuracy | Precision | Recall | F1 | ROC-AUC | Примечание |
|--------|----------|-----------|--------|----|---------|------------|
| Лучшая модель: Random Forest | 0.8267 | 0.6453 | 0.8193 | 0.7220 | 0.9056 | Лучшая модель после RandomizedSearchCV |
| Gradient Boosting | 0.8497 | 0.7603 | 0.6611 | 0.7072 | 0.9137 | RandomizedSearchCV, лучший ROC-AUC среди настроенных моделей |
| Decision Tree | 0.7628 | 0.5413 | 0.8949 | 0.6746 | 0.8809 | RandomizedSearchCV, высокий recall после настройки |
| Logistic Regression | 0.7594 | 0.5425 | 0.7924 | 0.6441 | 0.8520 | GridSearchCV по небольшой сетке параметров |
| KNeighborsClassifier | 0.8011 | 0.6641 | 0.5583 | 0.6066 | 0.8361 | RandomizedSearchCV |

Результаты лучшей модели на test-выборке:
| Модель | Accuracy | Precision | Recall | F1 | ROC-AUC | Примечание |
|--------|----------|-----------|--------|----|---------|------------|
| Random Forest | 0.8268 | 0.6443 | 0.8250 | 0.7235 | 0.9084 | Финальная оценка лучшей модели на отложенной test-выборке |
...
После подбора гиперпараметров основной прирост дала модель `RandomForestClassifier` (F1-score вырос с `0.5578` до `0.7220` на validation-выборке). Она стала финальной моделью и была дополнительно проверена на test-выборке, результаты: F1-score = `0.7235` и ROC-AUC = `0.9084`. Полные результаты CP2 сохранены в `data/processed/metrics_cp2.csv`, результаты тюнинга — в `data/processed/tuning_results_cp2.csv`, test-метрики — в `data/processed/test_metrics_cp2.csv`. Лучшая модель после тюнинга сохранена в `models/best_model_cp2.joblib`.
